...
def heartbeat_daemon(api_key):
    ...
    while True:
        try:
            ...
            # Instruct the agent to branch based on its own hostname
            evo_prompt = (
                f"You are the machine '{hostname}'. Execute the EVOLUTION PROTOCOL. "
                f"1. Create a branch: 'evolution-{hostname}'. "
                f"2. Invent and code a new feature in ~/GeminiMAS_Repo/bin/. "
                f"3. Push to GitHub. 4. Notify user via Telegram to /approve evolution-{hostname}."
            )
