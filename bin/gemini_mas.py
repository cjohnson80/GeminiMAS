...
REPO_ROOT = os.getcwd() # Evolution should focus on the local repo
...
class ResourceGuard:
    ...
    @staticmethod
    def safe_path(path):
        """Forces paths to stay within AGENT_ROOT or REPO_ROOT."""
        abs_p = os.path.abspath(os.path.expanduser(path))
        agent_root = os.path.abspath(AGENT_ROOT)
        repo_root = os.path.abspath(REPO_ROOT)
        
        if abs_p.startswith(agent_root) or abs_p.startswith(repo_root):
            return abs_p
        
        # Default to agent root to prevent leakage into ~
        return os.path.join(agent_root, "workspace", os.path.basename(abs_p))
        ...
        try:
            with Spinner(f"Executing {action}"):
                if action == "run_shell":
                    ...
                    p = ResourceGuard.safe_path(payload)
                    ...
                    p = ResourceGuard.safe_path(data['path'])
...
"""
    ...
    def triage(self, user_input):
        """Ultra-fast triage and complexity check."""
        lower_inp = user_input.lower()
        if len(user_input) < 20 and any(word in lower_inp for word in ["hi", "hello", "who are you", "help", "how are you"]):
            return "CHAT"
        
        prompt = f"Analyze: '{user_input}'. Is this a CHAT, a SIMPLE_TASK (one tool call), or a SWARM_TASK (complex)? Reply ONLY with the category name."
        res = self.client_lite.generate(prompt)
        res_upper = (res or "CHAT").upper()
        if "SWARM" in res_upper: return "SWARM_TASK"
        if "SIMPLE" in res_upper: return "SIMPLE_TASK"
        if "TASK" in res_upper: return "SIMPLE_TASK"
        return "CHAT"
...
"""
        ...
        # Token Efficiency: Limit context history to 4000 chars and tool results
        ...
        for attempt in range(8):
            ...
            if output and "{" in output and ("'tool'" in output.replace('"', "'") or '"tool"' in output):
                try:
                    ...
                    # Aggressive Summary for Token Efficiency
                    if len(tool_result) > 1500:
                        status("SUMMARY", f"Summarizing tool output ({len(tool_result)} chars)...", C_YELLOW)
                        tool_result = self.client_lite.generate(f"Summarize this tool output, focus on errors/results: {tool_result[:6000]}")
        ...
        brief_history = self.client_lite.generate(f"Summarize the attempts and failures so far to help a senior debugger: {history[-8000:]}")
        ...
        if advice:
            ...
            final_history = history[-6000:] + f"\n\nSENIOR_DEBUGGER_ADVICE: {advice}\nExecute the task one last time using this advice."
            return self.client_lite.generate(final_history, system_instruction=sys_prompt, images=images)

        return f"Task failed after 8 attempts."

    def fast_task(self, user_goal, images=None):
        """Execute simple tasks without the full swarm overhead."""
        status("FAST", "Executing simple task path...", C_GREEN)
        sys_instr = self.get_system_context()
        return self.run_worker_with_tools(user_goal, "Fast execution path.", sys_instr, role="Developer", images=images)

    def solve_task(self, user_goal, complexity="SWARM_TASK", images=None):
        if complexity == "SIMPLE_TASK":
            return self.fast_task(user_goal, images=images)
    ...
    def process(self, user_input, stream=False, images=None):
        ...
        if "TASK" in triage_res:
            response = self.solve_task(user_input, complexity=triage_res, images=images)
