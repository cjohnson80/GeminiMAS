#!/bin/bash
# GeminiMAS Universal Installer
# Run this script on any machine to install the Gemini Multi-Agent System v5.0

set -e

echo "==============================================="
echo " Installing GeminiMAS v5.0 (Elite Edition)"
echo "==============================================="

AGENT_ROOT="$HOME/gemini_agents"
mkdir -p "$AGENT_ROOT"/{workspace,memory,logs,core,bin,skills}

# 1. Write the SOUL file
...
# Active Goals
- [ ] Monitor project health and distill old memories into RULES.md to save RAM.
- [ ] Scan and index active codebases into DuckDB for instant recall.
EOF

# 3. Create the RULES file (Distilled Knowledge)
touch "$AGENT_ROOT/core/RULES.md"

# 4. Write the Python Core Engine (v5.0)
...
# 5. Create the global wrapper script
cat << 'EOF' > "$HOME/.local/bin/gagent"
#!/bin/bash
if [ -f "$HOME/Desktop/.env" ]; then
    export GEMINI_API_KEY=$(grep "GEMINI_API_KEY" "$HOME/Desktop/.env" | cut -d'"' -f2)
fi
...
echo " Setup Complete! "
echo "==============================================="
echo "To use on other computers, copy 'install_gemini_mas.sh' and run it."
