...
## 1. Overview
The `cli_gateway.py` serves as a local, interactive REPL (Read-Eval-Print Loop) alternative to the `tg_gateway.py`. It adheres to the **Gateway** and **Adapter** design patterns, ensuring the core `GeminiMAS` engine remains completely decoupled from the user interface.
...
- **Responsibility:** Acts as the translation layer between the raw CLI strings and the `GeminiMASWrapper`.
- **Mechanism:** Injects the `GeminiMASWrapper` as a dependency. When a user types a command, the adapter wraps the request in an async task, submits it to the `mas_wrapper.execute_task()`, and awaits the callback/future without blocking the REPL.
...
## 3. Integration Path
1. **Initialization:** The CLI Gateway boots, loads `.env` for the API key, and initializes `GeminiMASWrapper`.
